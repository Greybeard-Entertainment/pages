...
* GTK on Relm4
  ...
  One way to fix it is to do static linkage.  The other way is to package into a reproducible format (/e.g./ =nix=, or =guix=).  Finally as a last resort, one can package the problem into slightly (and only slightly) less defective packaging standards, snaps and flatpaks.
